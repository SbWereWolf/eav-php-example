...
(
    ...
    is_hidden INTEGER,
    insert_date TIMESTAMP WITH TIME ZONE DEFAULT now(),
    code CHAR(100) COLLATE pg_catalog."default",
    name VARCHAR(4000) COLLATE pg_catalog."default",
    description TEXT,
    city VARCHAR(200) COLLATE pg_catalog."default",
    country VARCHAR(200) COLLATE pg_catalog."default",
...
COMMENT ON COLUMN person_profile.id IS 'идентификатор профиля';
COMMENT ON COLUMN person_profile.code IS 'код';
COMMENT ON COLUMN person_profile.name IS 'имя';
COMMENT ON COLUMN person_profile.description IS 'описание';
COMMENT ON COLUMN person_profile.is_hidden IS 'флаг "является скрытым"';
COMMENT ON COLUMN person_profile.insert_date IS 'дата добавления записи';
COMMENT ON COLUMN person_profile.update_date IS 'дата последнего изменения записи';
COMMENT ON COLUMN person_profile.city IS 'город';
COMMENT ON COLUMN person_profile.country IS 'страна';
...
(
    ...
    CONSTRAINT fk_message_author_person_profile FOREIGN KEY (author) REFERENCES person_profile (id),
    CONSTRAINT fk_message_receiver_person_profile FOREIGN KEY (receiver) REFERENCES person_profile (id)
);
...
COMMENT ON COLUMN message.author IS 'идентификатор автора сообщения';
COMMENT ON COLUMN message.receiver IS 'идентификатор получателя сообщения';
COMMENT ON COLUMN message.message_text IS 'текст сообщения';
COMMENT ON COLUMN message.is_hidden IS 'флаг "является скрытым"';
COMMENT ON COLUMN message.date IS 'дата создания сообщения';
...
ALTER TABLE public.social_group
    OWNER to assay_manager;

COMMENT ON COLUMN social_group.id IS 'идентификатор группы';
COMMENT ON COLUMN social_group.is_hidden IS 'флаг "является скрытым"';
COMMENT ON COLUMN social_group.insert_date IS 'дата добавления записи';
COMMENT ON COLUMN social_group.name IS 'название группы';
COMMENT ON COLUMN social_group.description IS 'описание';

ALTER TABLE public.social_group
    OWNER to assay_manager;


DROP TABLE person_profile_social_group;

CREATE TABLE public.person_profile_social_group
(
    id SERIAL PRIMARY KEY NOT NULL,
    person_profile_id INTEGER,
    social_group_id INTEGER,
    CONSTRAINT fk_person_profile_id FOREIGN KEY (person_profile_id) REFERENCES person_profile (id),
    CONSTRAINT fk_social_group_id FOREIGN KEY (social_group_id) REFERENCES social_group (id)

 );

ALTER TABLE public.person_profile_social_group
    OWNER to assay_manager;

COMMENT ON COLUMN person_profile_social_group.id IS 'идентификатор';
COMMENT ON COLUMN person_profile_social_group.person_profile_id IS 'идентификатор профиля';
COMMENT ON COLUMN person_profile_social_group.social_group_id IS 'идентификатор социальной группы';

CREATE UNIQUE INDEX ux_person_profile_social_group_social_group_id_person_profile_id ON person_profile_social_group (social_group_id, person_profile_id);
